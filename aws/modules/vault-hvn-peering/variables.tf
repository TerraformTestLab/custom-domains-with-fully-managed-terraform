...
variable "hcp_api_address" {
  description = "HCP API hostname (no scheme) for the existing-routes lookup. The root module sources this from the HCP_API_HOST environment variable and asserts it is non-empty when manage_routes is true - never set it from a variables file or state. Only read when manage_routes is true."
  type        = string
  default     = ""
}
